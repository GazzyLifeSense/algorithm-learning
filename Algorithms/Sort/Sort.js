 ...
 *         （所有情况O(n) 非原地排序 空间O(max-min+1) 不稳定性）
 * 
 ...
 *           （时间、空间复杂度、稳定性取决于桶内排序算法 非原地排序）
 * 
 * Ⅹ 基数排序（基于桶排序）：非比较型整数排序算法，按关键字d(位数)划分m个桶进行排序（三位数时，d为3，m为10）；包括最低位优先法(LSD), 最高位优先法(MSD)两种方法。
 *                            （时间复杂度O(d(n + 2m))、空间复杂度O(n+m)、非原地排序、稳定性）
 */
...
//#region 基数排序：非比较型整数排序算法，按关键字d(位数)划分m个桶进行排序（三位数时，d为3，m为10）；包括最低位优先法(LSD), 最高位优先法(MSD)两种方法。
//                  （时间复杂度O(d(n + 2m))、空间复杂度O(n+m)、非原地排序、稳定性）
function radixSort(nums) {
    ...
    // 循环最大值的整数位数次
    while (max / exp > 1) {
        // 个位 x % 10 / 1; 十位 x % 100 / 10; 百位 x % 1000 / 100
        let buckets = new Array(10).fill(0).map(() => [])
        // 根据当前位放入对应桶中
        for (let i = 0; i < nums.length; i++) {
            let digit = Math.floor(nums[i] % (10 * exp) / exp)
...
// console.log(radixSort([53, 3, 542, 748, 14, 214]))
//#endregion